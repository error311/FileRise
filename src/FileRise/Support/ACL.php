...
class ACL
{
    ...
    public static function userHasAnyAccess(string $user, array $perms, string $folder = 'root'): bool
    {
        ...
        if (self::canReadOwn($user, $perms, $folder)) {
            return true;
        }
        if (self::normalizeFolder($folder) !== 'root') {
            return false;
        }
    ...
    public static function ownsFolderOrAncestor(string $user, array $perms, string $folder): bool
    {
        ...
        if (self::hasGrant($user, $folder, 'owners')) {
            return true;
        }
        ...
        while (count($parts) > 1) {
            ...
            if (self::hasGrant($user, $parent, 'owners')) {
                return true;
            }
        }
        return false;
        ...
        foreach ($map as $key => $entry) {
            ...
            if (is_array($entry)) {
                ...
                foreach (self::BUCKETS as $b) {
                    if (array_key_exists($b, $entry)) {
                        $clean[$b] = $entry[$b];
                    }
                }
                if (array_key_exists('inherit', $entry)) {
                    $clean['inherit'] = $entry['inherit'];
                }
                if (array_key_exists('explicit', $entry)) {
                    $clean['explicit'] = $entry['explicit'];
                }
                $entry = $clean ?: $entry;
            }
    ...
    public static function renameTree(string $oldFolder, string $newFolder): void
    {
        ...
        if ($old === '' || $old === 'root') {
            return; // nothing to re-key for root
        }
    ...
    private static function groupGrantsCap(array $grants, string $capKey): bool
    {
        ...
        switch ($capKey) {
            ...
            case 'read_own':
                ...
                if (!empty($grants['view'])) {
                    return true;
                }
                return !empty($grants['viewOwn']);
            case 'share_file':
                if (!empty($grants['share_file'])) {
                    return true;
                }
                return !empty($grants['shareFile']);
            case 'share_folder':
                if (!empty($grants['share_folder'])) {
                    return true;
                }
    ...
    private static function hasAnyExplicitEntry(string $user, string $folder): bool
    {
        ...
        if (is_array($explicit)) {
            foreach ($explicit as $k => $v) {
                ...
                if (strcasecmp($key, $user) === 0 && $v !== null) {
                    return true;
                }
            }
        }
    ...
    public static function isAdmin(array $perms = []): bool
    {
        ...
        if (isset($perms['role']) && (string)$perms['role'] === '1') {
            return true;
        }
    ...
    public static function hasGrant(string $user, string $folder, string $cap): bool
    {
        ...
        if (!$hasExplicitAny) {
            $ancestors = self::ancestors($folder);
            foreach ($ancestors as $ancestor) {
                if (!self::inheritFlag($ancestor, $user)) {
                    continue;
                }
    ...
    {
        if (self::canRead($user, $perms, $folder)) {
            return true;
        }
    ...
    public static function applyUserGrantsAtomic(string $user, array $grants): array
    {
        ...
        $fh = @fopen($path, 'c+');
        if (!$fh) {
            throw new RuntimeException('Cannot open ACL storage');
        }
        if (!flock($fh, LOCK_EX)) {
            fclose($fh);
        ...
        try {
            ...
            if (!isset($acl['groups'])  || !is_array($acl['groups'])) {
                $acl['groups']  = [];
            }
