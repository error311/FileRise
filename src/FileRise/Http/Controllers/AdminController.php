...
class AdminController
{
    ...
    private static function detectLegacyProBundle(): array
    {
        static $cache = null;
        static $cacheTs = 0;
        $now = time();
        if (is_array($cache) && ($now - $cacheTs) < 30) {
            return $cache;
        }
        ...
        $dir = rtrim((string)FR_PRO_BUNDLE_DIR, "/\\");
        if ($dir === '' || !is_dir($dir)) {
            $cache = $result;
            return $result;
        }
        ...
        foreach ($targets as $file) {
            ...
            if (
                strpos($raw, '/src/models/') !== false
                || strpos($raw, '\\src\\models\\') !== false
                || strpos($raw, '/src/controllers/') !== false
                || strpos($raw, '\\src\\controllers\\') !== false
            ) {
                $result['legacy'] = true;
                $result['files'][] = $file;
    ...
    public function getConfig(): void
    {
        ...
        $legacyBundle = ($isAdmin ? self::detectLegacyProBundle() : ['legacy' => false, 'files' => []]);
        ...
        $proInfo = [
            ...
            'bundleLegacy' => (bool)($legacyBundle['legacy'] ?? false),
            'bundleLegacyFiles' => (isset($legacyBundle['files']) && is_array($legacyBundle['files']))
                ? array_values($legacyBundle['files'])
                : [],
        ];
    ...
    private function installProBundleFromZip(string $zipPath, string $origName = '', ?string $workDir = null): array
    {
        ...
        try {
            ...
            // Core Pro classes live inside the bundle dir (./Pro)
            $proCoreRoot = $bundleRoot . DIRECTORY_SEPARATOR . 'Pro';
            ...
            for ($i = 0; $i < $zip->numFiles; $i++) {
                ...
                if (in_array($name, $allowedTopLevel, true)) {
                    ...
                    $category   = 'src';
                } elseif (strpos($name, 'src/FileRisePro/') === 0) {
                    // e.g. src/FileRisePro/Pro/ProGroups.php -> <bundle>/Pro/ProGroups.php
                    $relative = substr($name, strlen('src/FileRisePro/'));
                    if ($relative === '' || substr($relative, -1) === '/') {
                        continue;
                    }
                    $targetPath = $proCoreRoot . DIRECTORY_SEPARATOR . $relative;
                    $category   = 'src';
                } else {
                ...
                // Keep Pro classes co-located: rewrite shims to load from ./Pro instead of ../FileRisePro/Pro
                if ($category === 'src' && strpos($name, 'src/pro/') === 0) {
                    $data = str_replace('../FileRisePro/Pro/', 'Pro/', $data);
                }
